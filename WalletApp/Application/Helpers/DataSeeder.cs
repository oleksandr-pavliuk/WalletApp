    ...
    public static class DataSeeder
    {
        ...
        public static void Seed()
        {
            using (var _context = new ApplicationContext())
            {
                ...
                {
                    var user = new User() { Points = 1043, Balance = 1000 };

                    var transactionList = new List<Transaction>()
                {
                    new Transaction() { User = user, Bank = "monobank", Date = DateTime.Now, Description = "Purchases", Name = "Netflix",
                                        Sum = -120.00M, TransactionStatus = Status.Accepted },
                    new Transaction() { User = user, Bank = "wallet", Date = DateTime.Now, Description = "Income", Name = "Apple",
